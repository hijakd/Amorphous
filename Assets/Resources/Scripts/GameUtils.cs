...
public class GameUtils : MonoBehaviour {
    ...
    public static void SpawnNorthSouthWalls(List<Vector3> path, GameObject[] walls, Material material) {
        ...
        for (int i = 0; i < path.Count; i++) {
            if (i == 0) {
                ...
                Spawn(walls[GameData.south], path[i], material);
            }
    ...
    public static int FindLargestValue(List<int> values) {
        int tmpValue = GameData.westernEdge < GameData.southernEdge ? GameData.southernEdge : GameData.westernEdge;
        return values.Prepend(tmpValue).Max();
    }

    public static int FindSmallestValue(List<int> values) {
        int tmpValue = GameData.northernEdge < GameData.easternEdge ? GameData.easternEdge : GameData.northernEdge;
...
// #endif
    ...
    private static Vector3 ClampWithinBoundaries(Vector3 position) {
        if (position.x <= GameData.westernEdge) {
            position.x = GameData.westernEdge;
        }
        else if (position.x > GameData.easternEdge) {
            position.x = GameData.easternEdge;
        }

        if (position.z <= GameData.southernEdge) {
            position.z = GameData.southernEdge;
        }
        else if (position.z > GameData.northernEdge) {
            position.z = GameData.northernEdge;
        }
    ...
    private static Color AddColoursTogether(List<GameObject> waypoints) {
        ...
        GameData.hintColour01 = waypoints[waypoint01].gameObject.GetComponentInChildren<Renderer>().sharedMaterial.color;
        GameData.hintColour02 = waypoints[waypoint02].gameObject.GetComponentInChildren<Renderer>().sharedMaterial.color;
        var tmpColour = GameData.hintColour01 + GameData.hintColour02;
        return tmpColour;
    }
    ...
    public static List<Vector3> RemoveDuplicates(List<Vector3> pathList) {
        ...
        GameData.shortListed = true;
        return shortened.ToList();
    }

    public static IEnumerator WaitForRowSlice() {
        yield return new WaitUntil(() => GameData.firstRowFound == true);
    }

    public static IEnumerator WaitForColSlice() {
        yield return new WaitUntil(() => GameData.firstColFound == true);
    }

    public static IEnumerator WaitForListShortening() {
        yield return new WaitUntil(() => GameData.shortListed == true);
    }
